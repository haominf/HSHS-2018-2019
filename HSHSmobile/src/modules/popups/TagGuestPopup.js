    ...
    renderContent = () => {
        return (
            <View style={{
                ...
                <GuestFilter
                    filterOptions={{
                        age: {options: ['Old', 'Young', 'Middle'], color: 'rgba(119, 11, 22, 1)'},
                        gender: {options: ['F', 'M', "O"], color: '#4E84C4'}
                    }}
                    selectedFilters={(() => {
    ...
    render() {
        ...
        return (
            <Popup
                title={"Choose guests"}
                onConfirm={() => {
                    this.setState({checkedBefore: this.state.checkedNow.slice()});
                    this.props.onConfirm(this.state.checkedNow)
                }}
                onCancel={() => {
                    this.setState({checkedNow: this.state.checkedBefore.slice()});
                }}
                ref={(popup) => {
                    this.Popup = popup;
                }}
